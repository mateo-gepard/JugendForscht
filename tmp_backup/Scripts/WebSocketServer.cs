    ...
    private void HandleIsomerCommand(string action, int center)
    {
        ...
        if (action == "mirror")
        {
            if (library != null && library.GetCurrentMolecule() != null)
            {
                ...
                {
                    BroadcastMessage("{\"type\":\"error\",\"message\":\"Enantiomer konnte nicht erzeugt werden\"}");
                }
            }
            else
            {
        ...
        else if (action == "overlay")
        {
            ...
            {
                BroadcastMessage("{\"type\":\"error\",\"message\":\"Erst Enantiomer erzeugen\"}");
            }
        }
        else if (action == "invert")
        {
            // Phase 5: Diastereomer generation
            BroadcastMessage($"{{\"type\":\"status\",\"message\":\"Invertiere Zentrum {center}...\"}}");
        }
    }
